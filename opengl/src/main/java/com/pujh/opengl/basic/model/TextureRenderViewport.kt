...
//        Matrix.scaleM(this, 0, 0.5f, 0.5f, 1f)
    ...
    override fun sizeChange(width: Int, height: Int) {
        ...
        if (imageScale > windowScale) {
            viewportWidth = width
            viewportHeight = (width / imageScale).toInt()
        } else if (imageScale < windowScale) {
            viewportWidth = (height * imageScale).toInt()
            viewportHeight = height
        } else {
            viewportWidth = width
            viewportHeight = height
        }
        val offsetX = (width - viewportWidth) / 2
        val offsetY = (height - viewportHeight) / 2
        GLES30.glViewport(offsetX, offsetY, viewportWidth, viewportHeight)
    }
