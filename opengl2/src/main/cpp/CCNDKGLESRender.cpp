...
CCNDKGLESRender::CCNDKGLESRender() : m_pAssetManager(NULL) {

}

CCNDKGLESRender::~CCNDKGLESRender() {
    glDeleteTextures(6, m_texID);
}

void CCNDKGLESRender::InitGL() {
...
void CCNDKGLESRender::ResizeGL(int width, int height) {
...
void CCNDKGLESRender::SetupAssetManager(AAssetManager *pManager) {
    if (pManager == NULL) {
        return;
    }
...
void CCNDKGLESRender::loadTextureResources(AAssetManager *pManager) {
    for (int i = 0; i < 6; i++) {
        char nameBuff[6];
...
void CCNDKGLESRender::loadShaderResources(AAssetManager *pManager) {
}
